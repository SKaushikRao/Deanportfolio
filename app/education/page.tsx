            ...
            {qualifications.map((qual, idx) => (
              <Card key={idx} className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1 bg-[#3d3a37]/90 backdrop-blur-sm border-0 shadow-xl">
                <CardContent className="p-4 sm:p-6 lg:p-8">
                  <h3 className="text-lg sm:text-xl lg:text-2xl font-semibold text-[#e6b815] mb-2 sm:mb-3 group-hover:underline">{qual.degree}</h3>
                  <p className="text-base sm:text-lg text-white mb-1 sm:mb-2">{qual.institution}</p>
                  {qual.note && <p className="text-sm sm:text-base text-white/90 italic">{qual.note}</p>}
                </CardContent>
              </Card>
            ))}
          </div>
